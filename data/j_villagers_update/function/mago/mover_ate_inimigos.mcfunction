...
$execute if entity @n[tag=$(id).target,distance=..10] if score @s attack_cooldown matches 0 if predicate {condition:"minecraft:random_chance",chance:0.2} run function j_villagers_update:mago/congelamento
$execute if entity @n[tag=$(id).target,distance=..10] if score @s attack_cooldown matches 0 run function j_villagers_update:mago/magia
